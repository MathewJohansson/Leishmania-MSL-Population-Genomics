...
# MSL0-only plot (D2.4) commented out — requires dosage_filtered from Viking RData.
...
OUT_DIR  <- file.path(LOCAL_DIR, "Figures/Phylogenetics Work/NJ Trees/Whole-Genome/L. infantum-only")
DATA_OUT <- file.path(LOCAL_DIR, "Data/Phylogenetic_Data/NJ_Trees/Whole-Genome/L.infantum-only")
...
nj_tree <- read.tree(file.path(DATA_OUT, "diploid_whole_genome_Linfantum_only_Neis_tree.nwk"))
...
pop_membership <- read_tsv(file.path(LOCAL_DIR, "Data/population_structure/population_membership_table_infantum_only_k13_LI_notation.tsv"))
...
## 5. PLOT: MSL STATUS (panel A of D2.3) ---------------------------------------
...
## 6. PLOT: POPULATION (panel B of D2.3) ---------------------------------------
...
# D2.3: Combined MSL + Population panel
...
## 7. PLOT: MSL0-ONLY BY POPULATION (D2.4) -------------------------------------
...
## 9. PLOT: BY REGION (D2.2) ---------------------------------------------------
# Uses ALL tree tips — 90% filter not applied
# Region: Brazil → macro-region; all other countries → continent
